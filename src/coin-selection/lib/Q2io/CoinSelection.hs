...
import Cardano.Api qualified as Cardano.Api
...
import Q2io.UTxOCompatibility (
  UTxOCompatibility,
  compatibleWith,
  txCompatibility,
 )
import Q2io.Utils (mapError)
import Q2io.Utxos (
  BalanceChanges (..),
  UtxoSet (..),
 )
...
balancingError
  :: (MonadError BalancingError m) => Either (C.TxBodyErrorAutoBalance C.BabbageEra) a -> m a
...
balanceTransactionBody tracer systemStart eraHistory protocolParams stakePools CSInputs{csiUtxo, csiTxBody, csiChangeOutput, csiNumWitnesses = TransactionSignatureCount numWits} = do
  ...
  traceWith
    tracer
    StartBalancing
      { numInputs = csiTxBody ^. L.txIns . to length
      , numOutputs = csiTxBody ^. L.txOuts . to length
      }

  -- append output instead of prepending
  txbody0 <-
    balancingError . first C.TxBodyError $
      C.createAndValidateTransactionBody C.ShelleyBasedEraBabbage $
        csiTxBody & appendTxOut changeOutputSmall
  ...
  traceWith tracer $
    ExUnitsMap $
      fmap (second (first (C.docToString . C.prettyError))) $
  ...
  let txbodycontent1' =
        txbodycontent1 & set L.txFee (Coin (2 ^ (32 :: Integer) - 1)) & over L.txOuts (|> changeOutputLarge)

  -- append output instead of prepending
  txbody1 <-
    balancingError . first C.TxBodyError $
      C.createAndValidateTransactionBody C.ShelleyBasedEraBabbage txbodycontent1'
  ...
  txbody2 <-
    balancingError . first C.TxBodyError $
      C.createAndValidateTransactionBody C.ShelleyBasedEraBabbage txbodycontent2
  ...
  let !balance =
        view
          L._TxOutValue
          ( Cardano.Api.evaluateTransactionBalance
              C.ShelleyBasedEraBabbage
              (C.unLedgerProtocolParameters protocolParams)
              stakePools
              unregPoolStakeBalance
              mempty
              csiUtxo
              txbody2
          )
  ...
  changeOutputBalance <- case C.valueToLovelace balance of
    Just b -> do
      let op =
            csiChangeOutput
              & L._TxOut . _2 . L._TxOutValue . L._Value . at C.AdaAssetId <>~ (Just $ C.lovelaceToQuantity b)
  ...
  txbody3 <-
    balancingError . first C.TxBodyError $
      C.createAndValidateTransactionBody C.ShelleyBasedEraBabbage finalBodyContent

  balances <-
    maybe (throwError ComputeBalanceChangeError) pure (balanceChanges csiUtxo finalBodyContent)
...
appendTxOut
  :: C.TxOut C.CtxTx C.BabbageEra -> C.TxBodyContent C.BuildTx ERA -> C.TxBodyContent C.BuildTx ERA
...
balanceCheck pparams output =
  let balance = view (L._TxOut . _2) output
   in if view L._TxOutValue balance == mempty
        then return ()
        else do
          when
            (C.txOutValueToLovelace balance < 0)
            (balancingError $ Left $ C.TxBodyErrorAdaBalanceNegative $ C.txOutValueToLovelace balance)
...
balanceChanges (C.UTxO lookups) body = do
  ...
  inputs <-
    Utxos.invBalanceChange . foldMap (txOutChange . id)
      <$> traverse (\(txi, _) -> Map.lookup txi lookups) (body ^. L.txIns)
  pure (outputs <> inputs)

txOutChange :: TxOut ctx C.BabbageEra -> BalanceChanges
txOutChange
  ( view L._TxOut ->
      ( fmap C.fromShelleyPaymentCredential . preview (L._AddressInEra . L._Address . _2) -> Just addr
        , view L._TxOutValue -> value
        , _
        , _
        )
    ) =
...
signForWallet wallet (C.BalancedTxBody _ txbody _changeOutput _fee) =
  let wit =
        [ C.makeShelleyKeyWitness C.ShelleyBasedEraBabbage txbody $
            C.WitnessPaymentKey (Wallet.getWallet wallet)
        ]
...
addOwnInput body (Utxos.removeUtxos (spentTxIns body) -> UtxoSet{_utxos})
  ...
  | not (Map.null _utxos) =
      ...
      let availableUTxOs =
            List.sortOn
              (length . view (L._TxOut . _2 . L._TxOutValue . to C.valueToList) . fst . snd)
              (Map.toList _utxos)
...
setCollateral body (Utxos.onlyAda -> UtxoSet{_utxos}) =
  let noScripts = not (runsScripts body)
   ...
   in if noScripts || hasCollateral
        then pure body -- no script witnesses in inputs.
        else -- select the output with the largest amount of Ada
        case listToMaybe $
          List.sortOn (Down . C.selectLovelace . view (L._TxOut . _2 . L._TxOutValue) . fst . snd) $
            Map.toList _utxos of
...
runsScripts body =
  let scriptIns = body ^.. (L.txIns . traversed . _2 . L._BuildTxWith . L._ScriptWitness)
      ...
      withdrawals =
        body ^.. (L.txWithdrawals . L._TxWithdrawals . traversed . _3 . L._BuildTxWith . L._ScriptWitness)
...
balancePositive
  ...
  -> UtxoSet ctx a
  -> TxBodyContent BuildTx ERA
  -> m (TxBodyContent BuildTx ERA, C.TxOut C.CtxTx C.BabbageEra)
balancePositive dbg poolIds ledgerPPs utxo_ returnUTxO0 walletUtxo txBodyContent0 = do
  txb <-
    either
      (throwError . bodyError)
      pure
      (C.createAndValidateTransactionBody C.ShelleyBasedEraBabbage txBodyContent0)
  let bal =
        C.evaluateTransactionBalance
          C.ShelleyBasedEraBabbage
          (C.unLedgerProtocolParameters ledgerPPs)
          poolIds
          mempty
          mempty
          utxo_
...
addInputsForAssets
  ...
  -> UtxoSet ctx a
  -- ^ UTxOs that we can spend to cover the negative part of the balance
  -> TxBodyContent BuildTx ERA
  -- ^ Transaction body
...
addInputsForAssets dbg txBal availableUtxo txBodyContent
  ...
  | otherwise = do
      ...
      case Wallet.selectMixedInputsCovering availableUtxo missingAssets of
        Nothing ->
          throwError
            (NotEnoughMixedOutputsFor (C.valueFromList missingAssets) (Utxos.totalBalance availableUtxo) txBal)
...
addOutputForNonAdaAssets
  ...
  -> C.Value
  -- ^ The balance of the transaction
  -> (C.TxOut C.CtxTx C.BabbageEra, C.Quantity)
...
keyWitnesses
  :: (MonadBlockchain m)
  => C.TxBodyContent v C.BabbageEra
  -> m (Set (Keys.KeyHash 'Keys.Payment StandardCrypto))
...
requiredSignatureCount
  :: (MonadBlockchain m) => C.TxBodyContent C.BuildTx C.BabbageEra -> m TransactionSignatureCount
requiredSignatureCount content = do
  ...
  let hsh (C.PaymentKeyHash h) = h
      ...
      -}
      getCertKeyWits
        ( C.ShelleyRelatedCertificate
            _
            (TxCert.ShelleyTxCertDelegCert (TxCert.ShelleyUnRegCert (KeyHashObj hash)))
          ) =
          Set.singleton $ CertificateStakeKey hash
      getCertKeyWits
        ( C.ShelleyRelatedCertificate
            _
            (TxCert.ShelleyTxCertDelegCert (TxCert.ShelleyDelegCert (KeyHashObj hash) _))
          ) =
          Set.singleton $ CertificateStakeKey hash
...
-- TODO: consolidate with Q2io.BuildTx.spendPublicKeyOutput
