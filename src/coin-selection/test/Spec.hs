...
import Q2io.CardanoApi.Lenses qualified as L
import Q2io.Class (
  MonadBlockchain (..),
...
import Q2io.MockChain (
  ValidationError (..),
  failedTransactions,
  fromLedgerUTxO,
  runMockchain0IOWith,
 )
import Q2io.MockChain.CoinSelection (
  balanceAndSubmit,
  payToOperator',
  paymentTo,
  tryBalanceAndSubmit,
...
import Q2io.MockChain.Gen qualified as Gen
import Q2io.MockChain.Utils (
  mockchainSucceeds,
  runMockchainProp,
 )
import Q2io.NodeParams (ledgerProtocolParameters, maxTxSize)
import Q2io.Query (balancePaymentCredentials)
import Q2io.Utils (failOnError)
import Q2io.Utxos qualified as Utxos
import Q2io.Wallet (Wallet)
import Q2io.Wallet qualified as Wallet
import Q2io.Wallet.MockWallet qualified as Wallet
...
tests =
  testGroup
    ...
    , testGroup
        ...
        , testCase
            "spending a plutus script output"
            (mockchainSucceeds $ failOnError (payToPlutusScript >>= spendPlutusScript))
        , testCase
            "spending a plutus script (V2) output"
            (mockchainSucceeds $ failOnError (payToPlutusV2Script >>= spendPlutusV2Script))
        , testCase
            "creating a reference script output"
            (mockchainSucceeds $ failOnError $ putReferenceScript Wallet.w1)
        , testCase
            "using a reference script"
            (mockchainSucceeds $ failOnError (payToPlutusV2Script >>= spendPlutusScriptReference))
        , testCase "minting a token" (mockchainSucceeds $ failOnError mintingPlutus)
        , testCase
            "making payments with tokens"
            (mockchainSucceeds $ failOnError (mintingPlutus >>= spendTokens))
        , testCase
            "making payments with tokens (2)"
            (mockchainSucceeds $ failOnError (mintingPlutus >>= spendTokens2))
        , testCase
            "spending a singleton output"
            (mockchainSucceeds $ failOnError (mintingPlutus >>= spendSingletonOutput))
        , testCase
            "spend an output locked by the matching index script"
            (mockchainSucceeds $ failOnError matchingIndex)
        , testCase
            "mint a token with the matching index minting policy"
            (mockchainSucceeds $ failOnError matchingIndexMP)
        ]
    , testGroup
        "mockchain"
    ...
    , testGroup
        ...
        , testCase
            "zero withdrawal"
            (mockchainSucceeds $ failOnError $ registerStakingCredential >> withdrawZero)
        ]
    ]
...
payToPlutusScript = do
  let tx =
        execBuildTx'
          (payToPlutusV1 Defaults.networkId txInscript () C.NoStakeAddress (C.lovelaceToValue 10_000_000))
...
spendPlutusScript
  :: (MonadFail m, MonadMockchain m, MonadError BalanceTxError m) => C.TxIn -> m C.TxId
...
putReferenceScript
  :: (MonadFail m, MonadMockchain m, MonadError BalanceTxError m) => Wallet -> m C.TxIn
  ...
  let hsh = C.hashScript (C.PlutusScript C.PlutusScriptV2 Scripts.v2SpendingScript)
      addr =
        C.makeShelleyAddressInEra
          ...
          Defaults.networkId
          (C.PaymentCredentialByScript hsh)
          C.NoStakeAddress
      tx =
        execBuildTx' $
  ...
  let tx =
        execBuildTx'
          ( spendPlutusV2Ref
              txIn
              refTxIn
              (Just $ C.hashScript (C.PlutusScript C.PlutusScriptV2 Scripts.v2SpendingScript))
              ()
              ()
          )
  C.getTxId . C.getTxBody <$> tryBalanceAndSubmit mempty Wallet.w1 tx
...
-}
spendSingletonOutput
  :: (MonadFail m, MonadMockchain m, MonadError BalanceTxError m) => C.TxId -> m ()
spendSingletonOutput txi = do
  ...
  let totalVal = Utxos.totalBalance k
      newOut =
        C.TxOut
          (Wallet.addressInEra Defaults.networkId Wallet.w2)
          (C.TxOutValueShelleyBased C.ShelleyBasedEraBabbage $ C.toMaryValue totalVal)
          C.TxOutDatumNone
...
nativeAssetPaymentTo
  :: (MonadMockchain m, MonadFail m, MonadError BalanceTxError m)
  => C.Quantity
  -> Wallet
  -> Wallet
  -> m C.TxId
...
checkResolveDatumHash = do
  let addr = Wallet.addressInEra Defaults.networkId Wallet.w1
      assertDatumPresent vl =
        resolveDatumHash (C.hashScriptDataBytes vl) >>= \case
  ...
  _ <-
    tryExecBuildTxWallet
      Wallet.w1
      (payToPlutusV1 Defaults.networkId txInscript d2 C.NoStakeAddress mempty)
      ...
      txo =
        C.TxOut
          ( C.makeShelleyAddressInEra
              C.ShelleyBasedEraBabbage
              Defaults.networkId
              (C.PaymentCredentialByScript (C.hashScript (C.PlutusScript C.PlutusScriptV1 txInscript)))
              C.NoStakeAddress
          )
...
-}
tryExecBuildTxWallet
  :: (MonadMockchain m, MonadError BalanceTxError m, MonadFail m) => Wallet -> BuildTxT m a -> m C.TxId
                    ...
                    runMockchainProp $
                      lift $
                        failOnError $ do
                          ...
                          traverse_
                            (payToOperator' mempty (C.lovelaceToValue $ 2_500_000 + C.quantityToLovelace nAmount) Wallet.w2)
                            (op : operators)
                          ...
                          let tx = execBuildTx' $ do
                                ...
                                traverse_
                                  addRequiredSignature
                                  (fmap (C.verificationKeyHash . verificationKey . oPaymentKey) requiredSignatures)
                          ...
                          balancedTx <-
                            runExceptT
                              ( balancePaymentCredentials
                                  ...
                                  (operatorPaymentCredential <$> operators)
                                  Nothing
                                  tx
                              )
                              >>= either (fail . show) pure
                          txInputs <- let C.Tx (C.TxBody txBody) _ = balancedTx in keyWitnesses txBody
                          let (Set.fromList -> extraWits) =
                                let C.Tx (C.TxBody txBody) _ = balancedTx in view (L.txExtraKeyWits . L._TxExtraKeyWitnesses) txBody
                          ...
                          finalTx <- flip execStateT balancedTx $ flip traverse_ (op : operators) $ \o -> do
                            ...
                            when
                              ( pkh `Set.member` txInputs
                                  || (C.verificationKeyHash . verificationKey $ oPaymentKey o) `Set.member` extraWits
                              )
                              (modify (signTxOperator o))
                          void (sendTx finalTx)
...
buildTxMixedInputs = mockchainSucceeds $ failOnError $ do
  ...
  utxoSet <- Utxos.fromApiUtxo . fromLedgerUTxO C.ShelleyBasedEraBabbage <$> getUtxo
  let utxoVal =
        assetValue (C.hashScript $ C.PlutusScript C.PlutusScriptV1 mintingScript) "assetName" 40
          <> C.lovelaceToValue 10_000_000
      newUTxO =
        C.TxOut
          (Wallet.addressInEra Defaults.networkId testWallet)
          (C.TxOutValueShelleyBased C.ShelleyBasedEraBabbage $ C.toMaryValue utxoVal)
          C.TxOutDatumNone
          C.ReferenceScriptNone
...
largeTransactionTest = do
  let largeDatum :: [Integer] = replicate 10_000 33
      largeDatumTx =
        execBuildTxWallet
          Wallet.w1
          (payToPlutusV1 Defaults.networkId txInscript largeDatum C.NoStakeAddress mempty)
  ...
  runMockchain0IOWith Wallet.initialUTxOs Defaults.nodeParams (failOnError largeDatumTx) >>= \case
    Right (Left{}, view failedTransactions -> [(_, err)]) -> case err of
      ApplyTxFailure
        ( ApplyTxError
            (UtxowFailure (UtxoFailure (AlonzoInBabbageUtxoPredFailure (MaxTxSizeUTxO 20304 16384))) :| [])
          ) -> pure ()
      ...
      params' =
        Defaults.nodeParams
          & ledgerProtocolParameters
            .~ (either (error . show) id (C.convertToLedgerProtocolParameters C.ShelleyBasedEraBabbage protParams))
...
matchingIndex = do
  let txBody =
      ...
      tx =
        C.TxIn
          <$> (C.getTxId . C.getTxBody <$> tryBalanceAndSubmit mempty Wallet.w1 txBody)
          <*> pure (C.TxIx 0)
  ...
  void
    (tryBalanceAndSubmit mempty Wallet.w1 $ execBuildTx' $ traverse_ Scripts.spendMatchingIndex inputs)

stakingCredential :: C.StakeCredential
stakingCredential =
  C.StakeCredentialByScript $ C.hashScript (C.PlutusScript C.PlutusScriptV2 Scripts.v2StakingScript)

registerStakingCredential
  :: (MonadMockchain m, MonadError BalanceTxError m, MonadFail m) => m C.TxIn
...
withdrawZero = do
  txBody <-
    BuildTx.buildTx
      <$> execBuildTxT (BuildTx.addWithdrawZeroPlutusV2InTransaction Scripts.v2StakingScript ())
  txI <-
    C.TxIn
      <$> (C.getTxId . C.getTxBody <$> tryBalanceAndSubmit mempty Wallet.w1 txBody)
      <*> pure (C.TxIx 0)
...
matchingIndexMP = do
  ...
  void $
    tryBalanceAndSubmit mempty Wallet.w1 $
      execBuildTx' $
        traverse_ runTx ["assetName1", "assetName2", "assetName3"]
