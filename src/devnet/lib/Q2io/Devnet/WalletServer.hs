...
module Q2io.Devnet.WalletServer (
  RunningWalletServer (..),
  WalletLog (..),
  withWallet,
  getUTxOs,
...
import Q2io.Devnet.CardanoNode.Types (RunningNode (..))
import Q2io.Devnet.NodeQueries qualified as NodeQueries
import Q2io.Devnet.Utils (failure, withLogFile)
import Q2io.Devnet.Wallet qualified as Wallet
import Q2io.Utxos (UtxoSet)
import Q2io.Wallet.API qualified as API
import Q2io.Wallet.Cli.Command (CliCommand (..))
...
withWallet
  :: Tracer IO WalletLog -> FilePath -> RunningNode -> (RunningWalletServer -> IO a) -> IO a
withWallet tracer stateDirectory rn@RunningNode{rnNodeSocket, rnNodeConfigFile, rnNetworkId} action = do
  let logFilePath = stateDirectory </> "wallet-server.log"
      ...
      verificationConf =
        OperatorConfigVerification
          { ocvPaymentKeyFile = verificationKeyFile
          , ocvStakeVerificationKeyFile = Nothing
          }
  ...
  withLogFile logFilePath $ \out -> do
    ...
    withCreateProcess p{std_out = UseHandle out, std_err = UseHandle out} $
      \stdin stdout stderr processHandle -> do
        ...
        _ <-
          sendFundsToOperator tracer rn op (C.Quantity 100_000_000)
            >>= NodeQueries.waitForTxn rnNetworkId rnNodeSocket
...
sendFundsToOperator
  :: Tracer IO WalletLog -> RunningNode -> Operator k -> Quantity -> IO (Tx BabbageEra)
sendFundsToOperator tr node@RunningNode{rnNetworkId} op lvl = do
  let opAddress = operatorAddress rnNetworkId op
  Wallet.sendFaucetFundsTo
    (contramap WWallet tr)
    node
    (C.AddressInEra (C.ShelleyAddressInEra C.ShelleyBasedEraBabbage) opAddress)
    10
    lvl
