...
sendFaucetFundsTo tracer node destination n amount = do
  ...
  balanceAndSubmit
    ...
    fct
    ( BuildTx.execBuildTx' $
        replicateM n (BuildTx.payToAddress destination (C.lovelaceToValue $ C.quantityToLovelace amount))
    )
    []
...
createSeededWallet tracer node@RunningNode{rnNetworkId, rnNodeSocket} n amount = do
  ...
  sendFaucetFundsTo tracer node (Wallet.addressInEra rnNetworkId wallet) n amount
...
balanceAndSubmitReturn tracer node wallet returnOutput tx keys = do
  ...
  runningNodeBlockchain @String tracer node $ do
    (C.Tx body wit, _) <-
      failOnError (CoinSelection.balanceForWalletReturn mempty wallet utxos returnOutput tx)
