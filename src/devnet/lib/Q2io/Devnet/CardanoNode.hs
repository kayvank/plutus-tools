...
module Q2io.Devnet.CardanoNode (
  NodeLog (..),
  CardanoNodeArgs (..),
...
import Q2io.Devnet.NodeQueries qualified as Q
...
import Q2io.Wallet (Wallet, paymentCredential)
...
withCardanoNode tr networkId stateDirectory args@CardanoNodeArgs{nodeSocket, nodeConfigFile} action = do
  ...
  where
    ...
    waitForNode = do
      ...
      rnConnectInfo <-
        runExceptT (Q.loadConnectInfo rnNodeConfigFile socketPath)
          >>= either (error . (<>) "Failed to load connect info: " . show) pure
...
waitForNextBlock' node blockNo = do
  ...
  if currentBlockNo > blockNo
    then pure currentBlockNo
    else threadDelay 1_000_000 >> waitForNextBlock' node blockNo
...
waitForNextEpoch' n@RunningNode{rnNodeSocket, rnNetworkId} epochNo = do
  ...
  if currentEpochNo > epochNo
    then pure currentEpochNo
    else threadDelay 1_000_000 >> waitForNextEpoch' n epochNo
...
withCardanoStakePoolNodeDevnetConfig tracer stateDirectory wallet params nodeConfigFile PortsConfig{ours, peers} node@RunningNode{rnNodeSocket, rnNetworkId} action = do
  ...
  _ <-
    W.balanceAndSubmit
      ...
      wallet
      poolCertTx
      [C.WitnessStakeKey stakeKey, C.WitnessStakePoolKey stakePoolKey]
  _ <- waitForNextBlock node
