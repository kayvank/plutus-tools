...
import Q2io.NodeClient.ChainTip (
  JSONBlockNo (..),
  JSONChainPoint (..),
  JSONChainTip (..),
  blockHeaderPoint,
 )
import Q2io.NodeClient.Resuming (ResumingFrom)
import Q2io.NodeClient.Resuming qualified as R
      ...
      case pipelineDecisionMax pipelineSize n clientTip_ serverTip_ of
        ...
        _ ->
          CSP.SendMsgRequestNextPipelined
            (pure ())
            (clientIdle_RequestMoreN clientTip_ serverTip_ (Succ n) history)
    ...
    clientNextN n history =
      ClientStNext
                  ...
                  if newClientTip == newServerTip
                    then caughtUpWithNode serverChainTip
                      ...
                      catchingUpWithNode
                        (blockHeaderPoint bh)
                        (Just currBlockNo)
                        (Just $ chainTipToChainPoint serverChainTip)
        ...
        , recvMsgRollBackward = \chainPoint serverChainTip -> do
            ...
            !rolledBackState <-
              applyRollback chainPoint (foldMap (\(_, _, (s, _)) -> s) rolledBack) currentState
