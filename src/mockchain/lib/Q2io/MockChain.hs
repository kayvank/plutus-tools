...
import Q2io.Constants (ERA)
import Q2io.MockChain.Defaults ()
import Q2io.MockChain.Defaults qualified as Defaults
import Q2io.MonadLog (MonadLog (..))
import Q2io.NodeParams (NodeParams (..))
import Q2io.Utils (slotToUtcTime)
import Q2io.Utxos (
  UtxoSet (..),
  fromApiUtxo,
  onlyCredential,
...
fullyAppliedScript
  ...
  -> Either String (UPLC.Program UPLC.NamedDeBruijn UPLC.DefaultUni UPLC.DefaultFun ())
...
data MockChainState = MockChainState
  ...
  , mcsTransactions :: [(Validated (Core.Tx ERA), [PlutusWithContext StandardCrypto])]
  -- ^ Transactions that were submitted to the mockchain and validated
  , mcsFailedTransactions :: [(Tx BabbageEra, ValidationError)]
...
getTxExUnits NodeParams{npSystemStart, npEraHistory, npProtocolParameters} utxo (C.getTxBody -> tx) =
  case C.evaluateTransactionExecutionUnits
    C.BabbageEra
    npSystemStart
    (C.toLedgerEpochInfo npEraHistory)
    npProtocolParameters
    (fromLedgerUTxO C.ShelleyBasedEraBabbage utxo)
    tx of
...
applyTransaction params state tx'@(C.ShelleyTx _era tx) = do
  ...
  (vtx, scripts) <-
    first
      PredicateFailures
      (constructValidated (Defaults.globals params) (utxoEnv params currentSlot) utxoState_ tx)
...
evaluateTx
  ...
  -> UTxO ERA
  -> C.Tx C.BabbageEra
  -> Either ValidationError [PlutusWithContext StandardCrypto]
evaluateTx params slotNo utxo (C.ShelleyTx _ tx) = do
  (vtx, scripts) <-
    first
      PredicateFailures
      ( constructValidated
          (Defaults.globals params)
          (utxoEnv params slotNo)
          (lsUTxOState (mcsPoolState state))
          tx
      )
  ...
  where
    state =
      initialState params
        ...
        & poolState
          . L.utxoState
          . L._UTxOState (C.unLedgerProtocolParameters $ npProtocolParameters params)
          . _1
          .~ utxo
...
applyTx params oldState@MockChainState{mcsEnv, mcsPoolState} tx context = do
  (newMempool, vtx) <-
    first
      ApplyTxFailure
      (Cardano.Ledger.Shelley.API.applyTx (Defaults.globals params) mcsEnv mcsPoolState tx)
  return (oldState & poolState .~ newMempool & over transactions ((:) (vtx, context)), vtx)

newtype MockchainT m a
  = MockchainT (ReaderT NodeParams (StateT MockChainState (ExceptT MockchainError m)) a)
    ...
    C.UTxO mp <-
      gets
        ( view $
            poolState
              . L.utxoState
              . L._UTxOState (Defaults.pParams nps)
              . _1
              . to (fromLedgerUTxO C.ShelleyBasedEraBabbage)
        )
      ...
      rewardsMap =
        Map.fromList $
          bimap
            ...
            C.lovelaceToQuantity
            <$> Map.toList (rewardMap rewards')
      poolMap =
        Map.fromList $
          bimap
            fromLedgerStakeAddress
            StakePoolKeyHash
            <$> Map.toList (sPoolMap rewards')
...
runMockchainT
  :: MockchainT m a -> NodeParams -> MockChainState -> m (Either MockchainError (a, MockChainState))
...
runMockchain
  :: Mockchain a -> NodeParams -> MockChainState -> Either MockchainError (a, MockChainState)
...
runMockchain0With
  :: InitialUTXOs -> NodeParams -> Mockchain a -> Either MockchainError (a, MockChainState)
runMockchain0With dist params action = runMockchain action params (initialStateFor params dist)

evalMockchainT
  :: (Functor m) => MockchainT m a -> NodeParams -> MockChainState -> m (Either MockchainError a)
...
runMockchainIO
  :: MockchainIO a -> NodeParams -> MockChainState -> IO (Either MockchainError (a, MockChainState))
...
runMockchain0IOWith
  :: InitialUTXOs -> NodeParams -> MockchainIO a -> IO (Either MockchainError (a, MockChainState))
...
execMockchainIO
  :: MockchainIO a -> NodeParams -> MockChainState -> IO (Either MockchainError MockChainState)
