...
module Q2io.MockChain.Defaults (
  eraHistory,
  epochSize,
  slotLength,
  protocolParameters,
...
import Cardano.Api (envSecurityParam)
    ...
    one =
      nonEmptyHead $
        Ouroboros.getSummary $
          Ouroboros.neverForksSummary epochSize slotLength (GenesisWindow 4_000_000)
...
protocolParameters =
  ...
  let defaultV1CostModel =
   ...
   in ProtocolParameters
        ...
        , protocolParamPrices =
            Just
              (ExecutionUnitPrices{priceExecutionSteps = 721 % 10_000_000, priceExecutionMemory = 577 % 10_000})
        , protocolParamMaxTxExUnits =
            Just (ExecutionUnits{executionSteps = 1_0000_000_000, executionMemory = 16_000_000})
        , protocolParamMaxBlockExUnits =
            Just (ExecutionUnits{executionSteps = 4_0000_000_000, executionMemory = 80_000_000})
...
ledgerProtocolParameters =
  either (error . (<>) "ledgerProtocolParameters: toLedgerPParams failed with " . show) id $
    toLedgerPParams ShelleyBasedEraBabbage protocolParameters

globals :: NodeParams -> Globals
globals params@NodeParams{npProtocolParameters, npSlotLength} =
  mkShelleyGlobals
    (genesisDefaultsFromParams params)
    (fixedEpochInfo epochSize npSlotLength)
    ( fromMaybe (error "globals: Invalid version") $
        Version.mkVersion $
          fst $
            protocolParamProtocolVersion $
              C.fromLedgerPParams C.ShelleyBasedEraBabbage $
                C.unLedgerProtocolParameters npProtocolParameters
    )
...
bundledProtocolParameters =
  C.LedgerProtocolParameters $
    either (error . (<>) "toLedgerPParams failed: " . show) id $
      C.toLedgerPParams C.ShelleyBasedEraBabbage protocolParameters
