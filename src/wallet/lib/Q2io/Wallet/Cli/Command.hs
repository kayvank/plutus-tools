...
module Q2io.Wallet.Cli.Command (
  CliCommand (..),
  commandParser,
) where
...
import Q2io.Wallet.Cli.Config (Config, configParser)
...
data CliCommand
  ...
  | RunWallet Config OperatorConfigVerification PortNumber
...
generateSigningKey =
  command "generate-signing-key" $
    info
      (GenerateSigningKey <$> verificationKeyFileParser <*> signingKeyFileParser)
      (fullDesc <> progDesc "Generate a signing and verification key pair")

runWallet :: Mod CommandFields CliCommand
runWallet =
  command "run-wallet" $
    info
      (RunWallet <$> configParser <*> parseOperatorConfigVerification <*> portParser)
      (fullDesc <> progDesc "Start the wallet")

showAddress :: Mod CommandFields CliCommand
showAddress =
  command "show-address" $
    info
      (ShowAddress <$> configParser <*> parseOperatorConfigVerification)
      (fullDesc <> progDesc "Show the wallet's address")
