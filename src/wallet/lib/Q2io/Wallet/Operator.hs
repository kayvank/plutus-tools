...
import Q2io.Class (MonadBlockchain (networkId))
import Q2io.PlutusLedger (transPubKeyHash, transStakeKeyHash)
import Q2io.Utils (
  readSigningKeyFromFile,
  readStakingKeyFromFile,
  readVerificationKeyFromFile,
 )
import Q2io.Wallet (addSignature, addSignatureExtended)
...
operatorAddress networkId_ op =
  C.makeShelleyAddress
    ...
    ( maybe C.NoStakeAddress (C.StakeAddressByValue . C.StakeCredentialByKey . C.verificationKeyHash) $
        oStakeKey op
    )
...
loadOperatorFiles OperatorConfigSigning{ocSigningKeyFile, ocStakeVerificationKeyFile} =
  Operator
    <$> fmap PESigning (readSigningKeyFromFile ocSigningKeyFile)
    <*> traverse readStakingKeyFromFile ocStakeVerificationKeyFile
...
loadOperatorFilesVerification OperatorConfigVerification{ocvStakeVerificationKeyFile, ocvPaymentKeyFile} =
  Operator
    <$> fmap PEVerification (readVerificationKeyFromFile ocvPaymentKeyFile)
    <*> traverse readStakingKeyFromFile ocvStakeVerificationKeyFile
    ...
    <*> optional
      ( strOption
          ( long "stake-verification-key-file"
              <> metavar "FILE"
              <> help "The operator's stake verification key file (optional)."
          )
      )
...
parseOperatorConfigVerification =
  OperatorConfigVerification
    ...
    <*> optional
      ( strOption
          (long "stake-verification-key-file" <> help "Stake verification key of the operator (optional)")
      )
