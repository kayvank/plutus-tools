...
import Control.Lens (preview, view, _2)
import Data.Aeson (
  FromJSON (..),
  ToJSON (..),
  object,
...
import Q2io.Utxos (UtxoSet (..), onlyAda)
...
-}
addSignatureExtended
  :: forall era. (IsShelleyBasedEra era) => SigningKey PaymentExtendedKey -> C.Tx era -> C.Tx era
...
selectAnyInputsCovering UtxoSet{_utxos} (C.Quantity target) =
  let append
        (C.Quantity total_, txIns)
        ( txIn
          , C.lovelaceToQuantity . C.selectLovelace . view (L._TxOut . _2 . L._TxOutValue) -> C.Quantity coin_
