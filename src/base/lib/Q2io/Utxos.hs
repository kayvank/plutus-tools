...
-}
partition
  :: ((C.TxOut ctx C.BabbageEra, a) -> Bool) -> UtxoSet ctx a -> (UtxoSet ctx a, UtxoSet ctx a)
...
onlyCredentials cs =
  let flt
        ( fmap CS.fromShelleyPaymentCredential
            . preview (_1 . L._TxOut . _1 . L._ShelleyAddressInBabbageEra . _2) ->
            k
          ) = case k of
...
onlyStakeCredential (C.StakeAddressByValue -> c) =
  let flt
        ( fmap CS.fromShelleyStakeReference
            . preview (_1 . L._TxOut . _1 . L._ShelleyAddressInBabbageEra . _3) ->
            k
          ) = k == Just c
   in fst . partition flt
...
onlyPubKey =
  let flt =
        isJust
          . preview (_1 . L._TxOut . _1 . L._ShelleyAddressInBabbageEra . _2 . L._ShelleyPaymentCredentialByKey)
   in fst . partition flt
...
balanceChange UtxoChange{_outputsAdded, _outputsRemoved} =
  let k
        ( view (_1 . L._TxOut) ->
            ( fmap CS.fromShelleyPaymentCredential . preview (L._AddressInEra . L._Address . _2) -> Just addr
              , view L._TxOutValue -> vl
              , _
              , _
              )
          ) = Just (addr, vl)
...
describeChange UtxoChange{_outputsAdded, _outputsRemoved} =
  let tshow = Text.pack . show
   in tshow (Map.size _outputsAdded)
        <> " outputs added, "
        <> tshow (Map.size _outputsRemoved)
        <> " outputs removed"
...
-}
extract
  :: (C.TxIn -> C.TxOut C.CtxTx C.BabbageEra -> Maybe a)
...
extractBabbageTxn' ex UtxoSet{_utxos} cred theTx@(Tx txBody _) =
  let ShelleyTxBody _ txBody' _scripts scriptData _auxiliaryData _ = txBody
      ...
      checkInput
        :: (Word32, TxIn)
        -> Maybe (TxIn, ((C.TxOut C.CtxTx C.BabbageEra, a), Maybe (HashableScriptData, ExecutionUnits)))
      checkInput (idx, txIn) = fmap (txIn,) $ do
        o <- Map.lookup txIn _utxos
        let redeemer =
              fmap
                (bimap CS.fromAlonzoData CS.fromAlonzoExUnits)
                (Map.lookup (Scripts.AlonzoSpending $ Scripts.AsIx idx) txReds)
        pure (o, redeemer)

      checkOutput
        :: TxIx -> C.TxOut C.CtxTx C.BabbageEra -> Maybe (TxIn, (C.TxOut C.CtxTx C.BabbageEra, a))
