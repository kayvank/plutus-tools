...
import Q2io.Scripts qualified as Scripts
...
emptyTxOut addr =
  C.TxOut
    addr
    (C.lovelaceToTxOutValue C.ShelleyBasedEraBabbage 0)
    C.TxOutDatumNone
...
txProtocolParams
  :: Lens' (C.TxBodyContent v e) (BuildTxWith v (Maybe (C.LedgerProtocolParameters e)))
...
txProposalProcedures
  :: Lens'
      (C.TxBodyContent v BabbageEra)
      (Maybe (C.Featured C.ConwayEraOnwards BabbageEra (C.TxProposalProcedures v BabbageEra)))
...
txVotingProcedures
  :: Lens'
      (C.TxBodyContent v BabbageEra)
      (Maybe (C.Featured C.ConwayEraOnwards BabbageEra (C.TxVotingProcedures v BabbageEra)))
...
_TxWithdrawals
  :: Iso'
      (C.TxWithdrawals v BabbageEra)
      [(C.StakeAddress, Coin, BuildTxWith v (C.Witness C.WitCtxStake BabbageEra))]
...
_TxCertificates
  :: Iso'
      (C.TxCertificates BuildTx BabbageEra)
      ([C.Certificate BabbageEra], Map C.StakeCredential (C.Witness C.WitCtxStake BabbageEra))
...
_TxMintValue
  :: Iso' (TxMintValue BuildTx BabbageEra) (Value, Map PolicyId (ScriptWitness WitCtxMint BabbageEra))
_TxMintValue = iso from to
  where
    from
      :: TxMintValue BuildTx BabbageEra -> (Value, Map PolicyId (ScriptWitness WitCtxMint BabbageEra))
    from = \case
...
_TxOut
  :: Iso' (TxOut ctx era) (AddressInEra era, TxOutValue era, TxOutDatum ctx era, ReferenceScript era)
_TxOut = iso from to
  where
...
_ShelleyAddressInBabbageEra
  :: Prism'
      ...
      ( Shelley.Network
      , Credential.PaymentCredential StandardCrypto
      , Credential.StakeReference StandardCrypto
      )
_ShelleyAddressInBabbageEra = prism' from to
  where
    to
      :: C.AddressInEra C.BabbageEra
      -> Maybe
    ...
    from (ntw, pmt, stakeRef) =
      C.AddressInEra (C.ShelleyAddressInEra C.ShelleyBasedEraBabbage) (C.ShelleyAddress ntw pmt stakeRef)
...
_ShelleyPaymentCredentialByKey
  :: Prism' (Credential.PaymentCredential StandardCrypto) (Keys.KeyHash 'Keys.Payment StandardCrypto)
...
_ShelleyPaymentCredentialByScript
  :: Prism' (Credential.PaymentCredential StandardCrypto) (Hashes.ScriptHash StandardCrypto)
...
_ScriptWitness
  :: Prism' (C.Witness witctx era) (C.ScriptWitnessInCtx witctx, C.ScriptWitness witctx era)
...
_UTxOState pp = iso from to
  where
    ...
    to (utxosUtxo, utxosDeposited, utxosFees, utxosGovState, utxosDonation) =
      UTxOState
        { utxosUtxo
        , utxosDeposited
        , utxosFees
        , utxosGovState
        , utxosDonation
        , utxosStakeDistr = updateStakeDistribution pp mempty mempty utxosUtxo
        }
...
_HashableScriptData = prism' from to
  where
...
_PlutusScriptWitnessV1
  :: forall era witctx
   . Prism'
      ...
      ( C.ScriptLanguageInEra C.PlutusScriptV1 era
      , C.PlutusScriptVersion C.PlutusScriptV1
      , C.PlutusScriptOrReferenceInput C.PlutusScriptV1
...
_PlutusScriptWitnessV2
  :: forall era witctx
   . Prism'
      (C.ScriptWitness witctx era)
      ( C.ScriptLanguageInEra C.PlutusScriptV2 era
      , C.PlutusScriptVersion C.PlutusScriptV2
...
_TxValidityLowerBound
  :: forall era. Prism' (C.TxValidityLowerBound era) (C.AllegraEraOnwards era, C.SlotNo)
...
_TxValidityUpperBound
  :: forall era. Iso' (C.TxValidityUpperBound era) (C.ShelleyBasedEra era, Maybe SlotNo)
...
_TxValidityFiniteRange
  :: Prism' (C.TxValidityLowerBound C.BabbageEra, C.TxValidityUpperBound C.BabbageEra) (SlotNo, SlotNo)
_TxValidityFiniteRange = prism' from to
  where
    from (l, u) =
      ( C.TxValidityLowerBound C.AllegraEraOnwardsBabbage l
      , C.TxValidityUpperBound C.ShelleyBasedEraBabbage (Just u)
      )
    to = \case
