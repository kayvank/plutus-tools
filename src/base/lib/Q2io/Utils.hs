...
import Q2io.MonadLog (MonadLog, logWarnS)
import Q2io.PlutusLedger (
  transPOSIXTime,
  unTransPOSIXTime,
 )
...
scriptFromCborV1 cbor = do
  ...
  C.deserialiseFromTextEnvelope (C.proxyToAsType $ Proxy @(PlutusScript PlutusScriptV1)) textEnvelope
    & first show
...
scriptAddressV1 network script =
  let hash = C.hashScript (C.PlutusScript C.PlutusScriptV1 script)
   in C.makeShelleyAddressInEra
        ...
        network
        (C.PaymentCredentialByScript hash)
        C.NoStakeAddress
...
scriptFromCbor cbor = do
  ...
  textEnvelope <- fromJSON vl & (\case Error err -> Left (show err); Success e -> Right e)
  C.deserialiseFromTextEnvelope (C.proxyToAsType $ Proxy @(PlutusScript PlutusScriptV2)) textEnvelope
    & first show
...
utcTimeToSlot
  :: C.EraHistory -> C.SystemStart -> UTCTime -> Either String (SlotNo, NominalDiffTime, NominalDiffTime)
...
utcTimeToSlotUnsafe (C.EraHistory interpreter) systemStart t =
  first show $
    Qry.interpretQuery
      (Qry.unsafeExtendSafeZone interpreter)
      (Qry.wallclockToSlot (Time.toRelativeTime systemStart t))
...
readKeyFromFile
  :: (C.SerialiseAsBech32 key, C.HasTextEnvelope key) => Proxy key -> FilePath -> IO key
  ...
  case C.deserialiseFromBech32 (C.proxyToAsType p) txt of
    Left err1 ->
      C.readFileTextEnvelope (C.proxyToAsType p) (C.File source) >>= \case
        Left err2 ->
          fail
            ("readKeyFromFile: Failed to read " <> source <> ". Errors: " <> show err1 <> ", " <> show err2)
        Right k -> pure k
    Right k -> pure k
