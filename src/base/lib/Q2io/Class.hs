...
class (Monad m) => MonadBlockchain m where
  ...
  sendTx
    :: Tx BabbageEra
    -> m (Either SendTxFailed TxId)
    -- ^ Submit a transaction to the network

  utxoByTxIn
    :: Set C.TxIn
    -> m (C.UTxO C.BabbageEra)
    -- ^ Resolve tx inputs
...
setPOSIXTime tm =
  (posixTimeToSlotUnsafe <$> queryEraHistory <*> querySystemStart <*> pure tm)
    >>= either fail (setSlot . view _1)
...
instance (Monad m) => MonadError e (MonadBlockchainCardanoNodeT e m) where
  ...
  catchError (MonadBlockchainCardanoNodeT action) handler =
    MonadBlockchainCardanoNodeT $
      catchError
        action
        (\case MonadBlockchainError e -> unMonadBlockchainCardanoNodeT (handler e); e' -> throwError e')

runMonadBlockchainCardanoNodeT
  :: LocalNodeConnectInfo
  -> MonadBlockchainCardanoNodeT e m a
  -> m (Either (MonadBlockchainError e) a)
runMonadBlockchainCardanoNodeT info (MonadBlockchainCardanoNodeT action) =
  runExceptT (runReaderT action info)
...
instance (MonadLog m, MonadIO m) => MonadBlockchain (MonadBlockchainCardanoNodeT e m) where
  ...
  queryStakeAddresses creds nid =
    first (fmap C.lovelaceToQuantity)
      <$> runQuery'
        (C.QueryInEra (C.QueryInShelleyBasedEra C.ShelleyBasedEraBabbage (C.QueryStakeAddresses creds nid)))
