...
import Q2io.Lenses qualified as L
import Q2io.MonadLog (
  MonadLog (..),
  MonadLogIgnoreT,
  MonadLogKatipT,
 )
import Q2io.Scripts (toHashableScriptData)
...
lookupIndexWithdrawal stakeAddress =
  Set.lookupIndex stakeAddress
    . Set.fromList
    . fmap (view _1)
    . view (L.txWithdrawals . L._TxWithdrawals)
...
-}
addInputWithTxBody
  :: (MonadBuildTx m) => C.TxIn -> (TxBody -> C.Witness WitCtxTxIn C.BabbageEra) -> m ()
...
addMintWithTxBody
  ...
  -> C.AssetName
  -> C.Quantity
  -> (TxBody -> C.ScriptWitness C.WitCtxMint C.BabbageEra)
  -> m ()
addMintWithTxBody policy assetName quantity f =
  let v = assetValue (C.unPolicyId policy) assetName quantity
   in addTxBuilder
        ( TxBuilder $ \body -> (over (L.txMintValue . L._TxMintValue) (over _1 (<> v) . over _2 (Map.insert policy (f body))))
        )
...
addWithdrawalWithTxBody
  ...
  => C.StakeAddress
  -> C.Quantity
  -> (TxBody -> C.Witness C.WitCtxStake C.BabbageEra)
  -> m ()
addWithdrawalWithTxBody address amount f =
  addTxBuilder
    ( TxBuilder $ \body ->
        ( over
            (L.txWithdrawals . L._TxWithdrawals)
            ((address, C.quantityToLovelace amount, C.BuildTxWith $ f body) :)
        )
    )
...
spendPlutusV1
  :: forall datum redeemer m
   . (MonadBuildTx m, Plutus.ToData datum, Plutus.ToData redeemer)
  => C.TxIn
  ...
  let wit =
        C.BuildTxWith $
          C.ScriptWitness C.ScriptWitnessForSpending $
            buildV1ScriptWitness s (C.ScriptDatumForTxIn dat) red
...
spendPlutusV2 txIn s (toHashableScriptData -> dat) red =
  let wit =
        C.BuildTxWith $
          C.ScriptWitness C.ScriptWitnessForSpending $
            buildV2ScriptWitness s (C.ScriptDatumForTxIn dat) red
   in setScriptsValid >> addBtx (over L.txIns ((txIn, wit) :))

-- | Spend an output locked by a Plutus V2 validator with an inline datum
spendPlutusV2InlineDatum
  :: forall redeemer m
   . (MonadBuildTx m, Plutus.ToData redeemer)
  => C.TxIn
  -> PlutusScript PlutusScriptV2
  -> redeemer
  -> m ()
spendPlutusV2InlineDatum txIn s red =
  let wit =
        C.BuildTxWith $
          C.ScriptWitness C.ScriptWitnessForSpending $
            buildV2ScriptWitness s C.InlineScriptDatum red
   in setScriptsValid >> addBtx (over L.txIns ((txIn, wit) :))
...
spendPlutusV2RefBase txIn refTxIn sh dat red =
  let wit txBody =
        C.BuildTxWith $
          C.ScriptWitness C.ScriptWitnessForSpending $
            buildV2RefScriptWitness refTxIn sh dat (red $ findIndexSpending txIn txBody)
...
mintPlutusV1
  :: forall redeemer m
   . (Plutus.ToData redeemer, MonadBuildTx m)
  => PlutusScript PlutusScriptV1
  -> redeemer
  -> C.AssetName
  -> C.Quantity
...
payToAddressTxOut addr vl =
  C.TxOut
    addr
    (C.TxOutValueShelleyBased C.ShelleyBasedEraBabbage $ C.toMaryValue vl)
    C.TxOutDatumNone
  ...
  let val = C.TxOutValueShelleyBased C.ShelleyBasedEraBabbage $ C.toMaryValue vl
      addr =
        C.makeShelleyAddressInEra
          C.ShelleyBasedEraBabbage
          network
          (C.PaymentCredentialByScript script)
          stakeAddress
...
payToPlutusV1
  :: forall a m
   . (MonadBuildTx m, Plutus.ToData a)
  => NetworkId
  -> PlutusScript PlutusScriptV1
  -> a
  -> C.StakeAddressReference
...
payToPlutusV2InlineBase addr script dat vl =
  let refScript =
        C.ReferenceScript
          ...
          (C.toScriptInAnyLang $ C.PlutusScript C.PlutusScriptV2 script)
      txo = C.TxOut addr (C.TxOutValueShelleyBased C.ShelleyBasedEraBabbage $ C.toMaryValue vl) dat refScript
   in prependTxOut txo

payToPlutusV2Inline
  :: (MonadBuildTx m) => C.AddressInEra C.BabbageEra -> PlutusScript PlutusScriptV2 -> C.Value -> m ()
...
payToPlutusV2InlineDatum network script datum stakeRef vl =
  let val = C.TxOutValueShelleyBased C.ShelleyBasedEraBabbage $ C.toMaryValue vl
      ...
      addr =
        C.makeShelleyAddressInEra C.ShelleyBasedEraBabbage network (C.PaymentCredentialByScript sh) stakeRef
...
setMinAdaDeposit params txOut =
  let minUtxo = minAdaDeposit params txOut
   in txOut
        & over
          (L._TxOut . _2 . L._TxOutValue . L._Value . at C.AdaAssetId)
          (maybe (Just minUtxo) (Just . max minUtxo))

{- | Calculate the minimum amount of Ada that must be locked in the given UTxO to
satisfy the ledger's minimum Ada constraint.
-}
minAdaDeposit
  :: C.LedgerProtocolParameters C.BabbageEra -> C.TxOut C.CtxTx C.BabbageEra -> C.Quantity
  ...
  let minAdaValue = C.Quantity 3_000_000
      txo =
        txOut
          ...
          & over
            (L._TxOut . _2 . L._TxOutValue . L._Value . at C.AdaAssetId)
            (maybe (Just minAdaValue) (Just . max minAdaValue))
...
addWithdrawal
  :: (MonadBuildTx m) => C.StakeAddress -> C.Quantity -> C.Witness C.WitCtxStake C.BabbageEra -> m ()
...
-}
addScriptWithdrawal
  :: (MonadBlockchain m, MonadBuildTx m)
  => ScriptHash
  -> C.Quantity
  -> C.ScriptWitness C.WitCtxStake C.BabbageEra
  -> m ()
...
-}
addWithdrawZeroPlutusV2InTransaction
  :: (MonadBlockchain m, MonadBuildTx m, Plutus.ToData redeemer)
  => PlutusScript PlutusScriptV2
  -> redeemer
  -> m ()
...
addWithdrawZeroPlutusV2Reference
  ...
  -> redeemer
  -> m ()
addWithdrawZeroPlutusV2Reference refTxIn script redeemer =
  addScriptWithdrawal script 0 $
    buildV2RefScriptWitness refTxIn (Just script) C.NoScriptDatumForStake redeemer
...
addStakeCredentialCertificate =
  addCertificate
    . C.ShelleyRelatedCertificate C.ShelleyToBabbageEraBabbage
    . TxCert.RegTxCert
    . C.toShelleyStakeCredential

-- | Add a stake witness to the transaction
addStakeWitness
  :: (MonadBuildTx m) => C.StakeCredential -> C.Witness C.WitCtxStake C.BabbageEra -> m ()
