...
newtype MonadLogIgnoreT m a = MonadLogIgnoreT {runMonadLogIgnoreT :: m a}
  deriving newtype
    (Functor, Applicative, Monad, MonadIO, MonadCatch, MonadThrow, MonadMask, MonadFail)
...
withKatipLogging severity environment initialNamespace action = do
  handleScribe <-
    Katip.mkHandleScribe Katip.ColorIfTerminal stdout (Katip.permitItem severity) Katip.V2
  let makeLogEnv =
        Katip.registerScribe (fromString "stdout") handleScribe Katip.defaultScribeSettings
          =<< Katip.initLogEnv initialNamespace environment
